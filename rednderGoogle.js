...
const apiData = async () => {
  ...
  const getAllEventsGoogle = await fetch(
    "https://www.googleapis.com/calendar/v3/calendars/mohamed.nasser21794@gmail.com/events?key=AIzaSyC-40AiTBDvPeeMT_iRU7zaLDkxJ_skTK4",
    { method: "GET" }
  );
  const allEventsGoogle = await getAllEventsGoogle.json();
  allEventsGoogle.items.forEach(event => {
    ...
    allEvents.events.push(
        {
            ...
            interval: 1,
            byweekday: weekday,
            until: (event.recurrence != undefined) ? event.end.dateTime : event.start.dateTime,
            }
        }
    )
  });
    
//   console.log(allEventsGoogle.items);
